...
public class Tests
{
    ...
    private void Test3(IShip ship, Collection<IEnvironment> environments, Result.Results expectedValue)
    {
        Assert.Equal(expectedValue, new Runner(ship, environments).Run().FinalMessage);
    }
    ...
    private void Test7(IShip ship, Collection<IEnvironment> environments, Result.Results expectedValue)
    {
        Assert.Equal(expectedValue, new Runner(ship, environments).Run().FinalMessage);
    }
}